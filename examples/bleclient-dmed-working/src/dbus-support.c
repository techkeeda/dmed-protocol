...
OrgBluezGattCharacteristic1 *dmed_chr_iface;
		...
		GDBusMethodInvocation *invocation)
{
	...
	/* DMED Card Service */
	g_variant_builder_init(&child, G_VARIANT_TYPE_ARRAY);
	properties = dmed_get_svc_properties();
	g_variant_builder_add(&child, "{s@a{sv}}", SERVICE_INTERFACE, properties);
	variant = g_variant_builder_end(&child);
	g_variant_builder_add(&parent, "{o@a{sa{sv}}}", DMED_SVC_CARD_PATH, variant);

	g_variant_builder_init(&child, G_VARIANT_TYPE_ARRAY);
	properties = dmed_get_chr_properties();
	g_variant_builder_add(&child, "{s@a{sv}}", CHARACTERISTICS_INTERFACE, properties);
	variant = g_variant_builder_end(&child);
...
static gboolean on_dmed_get_all_properties(OrgFreedesktopDBusProperties *object,
	GDBusMethodInvocation *invocation, const gchar *arg_interface)
{
	const gchar *method_obj_path = g_dbus_method_invocation_get_object_path(invocation);
	GVariant *dict_variant = NULL;

	if (g_strcmp0(arg_interface, SERVICE_INTERFACE) == 0) {
		if (g_strcmp0(method_obj_path, DMED_SVC_CARD_PATH) == 0)
			dict_variant = dmed_get_svc_properties();
	} else if (g_strcmp0(arg_interface, CHARACTERISTICS_INTERFACE) == 0) {
		if (g_strcmp0(method_obj_path, DMED_CHR_DATA_PATH) == 0)
			dict_variant = dmed_get_chr_properties();
	}
...
void on_bus_acquired(GDBusConnection *connection, const gchar *name, gpointer userdata)
{
	...
	dmed_chr_iface = org_bluez_gatt_characteristic1_skeleton_new();

	OrgFreedesktopDBusProperties *dmed_chr_props = org_freedesktop_dbus_properties_skeleton_new();

	g_signal_connect(dmed_card_svc_props, "handle-get-all", G_CALLBACK(on_dmed_get_all_properties), 0);
	g_signal_connect(dmed_chr_props, "handle-get-all", G_CALLBACK(on_dmed_get_all_properties), 0);

	g_signal_connect(dmed_chr_iface, "handle-write-value", G_CALLBACK(dmed_on_write), 0);
	g_signal_connect(dmed_chr_iface, "handle-start-notify", G_CALLBACK(dmed_on_start_notify), 0);
	g_signal_connect(dmed_chr_iface, "handle-stop-notify", G_CALLBACK(dmed_on_stop_notify), 0);

	g_dbus_interface_skeleton_export(G_DBUS_INTERFACE_SKELETON(dmed_card_svc_props), connection, DMED_SVC_CARD_PATH, &error);
	g_dbus_interface_skeleton_export(G_DBUS_INTERFACE_SKELETON(dmed_chr_iface), connection, DMED_CHR_DATA_PATH, &error);
	g_dbus_interface_skeleton_export(G_DBUS_INTERFACE_SKELETON(dmed_chr_props), connection, DMED_CHR_DATA_PATH, &error);
